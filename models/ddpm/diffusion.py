...
import h_classification
import multi_classifier
...
class DDPM(nn.Module):
    ...
    def forward(self, x, t, sample = True, male = 1, eyeglasses = 1, scale = 1.7, index=None, t_edit=400, hs_coeff=(1.0, 1.0), delta_h=None, ignore_timestep=False, use_mask=False):
        ...
        # Change the value of h to be that of the other image here
        middle_h = h
        h2 = None
        ...
        # # Applying classification guidance
        # if 183<t.item()<814:
        # #     # print("---------------")
        # #     # print("classification guidance")
        #     # gradients, females = h_classification.gradients_point_five(h,int(t.item()), '/data/abhijnya/Classifier_guidance/classification_checkpoints/gender_linear.pt')
        #     # gradients, females = h_classification.gradients_point_five(h,int(t.item()), '/data/abhijnya/Classifier_guidance/classification_checkpoints/eyeglasses_linear.pt')
        #     # gradients = h_classification.gradients(h,int(t.item()), male=1, eyeglasses=1)
        #     # gradients, females = multi_classifier.gradients_point_five(h,int(t.item()))
            
            
        #     # print(females)
        # #     # gradients = h_classification.gradients(h,int(t.item()),1, '/data/abhijnya/Classifier_guidance/classification_checkpoints/gender_linear_sigmoid.pt')
        # #     # gradients, males = h_classification.gradients_fraction(h,int(t.item()), '/data/abhijnya/Classifier_guidance/classification_checkpoints/gender_linear_sigmoid.pt')
        #     # to_return = None

        #     if sample:
        #         # print("sampling")
        #         # print(male, eyeglasses)
        #         # gradients = h_classification.gradients(h,int(t.item()), male=male, eyeglasses=eyeglasses)
        #         gradients = h_classification.gradients_og(h,int(t.item()))

        #     else:
        #         # print("distribution")
        #         gradients, females = multi_classifier.gradients_point_five(h,int(t.item()), male=male, eyeglasses=eyeglasses)
        ...
        #         # print(scale)
        #         h = h.detach() - gradients*scale

                

                # to_return = gradients.view(gradients.size(0), -1).mean(1)
                # to_return = torch.norm(gradients.view(gradients.size(0), -1), dim = 1)


                # if t.item() in [0, 183, 387, 591, 795]:
                #     print("saving cuz", t.item())
                    # torch.save(gradients.view(gradients.size(0), -1).mean(1), f"/data/abhijnya/Classifier_guidance/logits/{t.item()}.pt")
                # if t.item()==0:
                #     to_return = gradients.view(gradients.size(0), -1).mean(1)
                #     self.count+=1
                # if t.item()==183:
                #     torch.save(gradients, f"/data/abhijnya/Classifier_guidance/logits/10/{self.count}.pt")
                # if t.item()==387:
                #     torch.save(gradients, f"/data/abhijnya/Classifier_guidance/logits/20/{self.count}.pt")
                # if t.item()==591:
                #     torch.save(gradients, f"/data/abhijnya/Classifier_guidance/logits/30/{self.count}.pt")
                # if t.item()==795:
                #     to_return = gradients.view(gradients.size(0), -1).mean(1)
            
            # # For generating only males
            # gradients = h_classification.gradients(h,int(t.item()),1, '/data/abhijnya/Classifier_guidance/classification_checkpoints/gender_linear.pt')[0]
            # h = h.detach() - gradients*self.scaling

            # # For generating only males
            # gradients, indices = h_classification.gradients_equal_extra(h,int(t.item()), '/data/abhijnya/Classifier_guidance/classification_checkpoints/gender_linear.pt')

            # if indices is not None:
            #     h[[indices]] = h[[indices]].detach() - gradients*self.scaling

        # if t.item()==0:
        #     males = h_classification.gradients(h,int(t.item()),1, '/data/abhijnya/Classifier_guidance/classification_checkpoints/gender_linear.pt')
        #     torch.save(males, f"/data/abhijnya/Classifier_guidance/logits/0/{self.count}.pt")
        #     print("saving")
        #     self.count+=1
        # if t.item()==387:
        #     males = h_classification.gradients(h,int(t.item()),1, '/data/abhijnya/Classifier_guidance/classification_checkpoints/gender_linear.pt')
        #     torch.save(males, f"/data/abhijnya/Classifier_guidance/logits/20/{self.count}.pt")
        #     print("saving")
        # if t.item()==591:
        #     males = h_classification.gradients(h,int(t.item()),1, '/data/abhijnya/Classifier_guidance/classification_checkpoints/gender_linear.pt')
        #     torch.save(males, f"/data/abhijnya/Classifier_guidance/logits/30/{self.count}.pt")
        #     print("saving")
        # if t.item()==795:
        #     males = h_classification.gradients(h,int(t.item()),1, '/data/abhijnya/Classifier_guidance/classification_checkpoints/gender_linear.pt')
        #     torch.save(males, f"/data/abhijnya/Classifier_guidance/logits/40/{self.count}.pt")
        #     print("saving")
        ...
        # if 183<t.item()<814:
        #     return h, h2, females, middle_h
        
        return h, h2, None, middle_h
