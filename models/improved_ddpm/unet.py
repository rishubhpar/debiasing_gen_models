...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, y=None, index=None, t_edit=400, hs_coeff=(1.0, 1.0), delta_h=None, ignore_timestep=False, sample = True, male = 1, eyeglasses = 1, scale = [1500], timestep_list = [1,50], use_mask=False, usefancy=False, bt=1, gamma_factor=0.1, guidance_loss='chi_without_5', attribute_list=[0,1,0,0], vanilla_generation=False):
        """
        ...
        if not vanilla_generation:
            
            if conversion_timestep(timestep_list[0]) <= timesteps.item() <= conversion_timestep(timestep_list[1]):

                if sample:
                    # print("sampling")
                    # print(male, eyeglasses)
                    gradients = h_classification.gradients(h,int(timesteps.item()), male=male, eyeglasses=eyeglasses, attribute_list=attribute_list,scale=scale)
                    # gradients = h_classification.flipping_imagenet(h,int(timesteps.item()),scale=scale)

                    # gradients = h_classification.gradients_og(h,int(timesteps.item()))

                else:
                    # print("distribution")
                    gradients = multi_classifier.gradients_point_five(h,int(timesteps.item()), male=male, eyeglasses=eyeglasses, guidance_loss=guidance_loss, attribute_list=attribute_list, scale=scale)
                ...
                if gradients is not None:
                    ...
                    if usefancy:
                        ...
                        # scales = torch.tensor(np.arange(1,-0.1,-0.1)).reshape(11,1,1,1)
                        ...
                        # print(int(timesteps.item()))
                        # h = th.randn((h.shape)).to(h.device)
                        # h = th.zeros((h.shape)).to(h.device)
                        # print((middle_h==h).unique())
                        # h = gradients
...
def conversion_timestep(t):
    array = [0, 20, 40, 61, 81, 101, 122, 142, 163, 183, 203, 224, 244, 265, 285, 305, 326, 346, 366, 387, 407, 428, 448, 468, 489, 509, 530, 550, 570, 591, 611, 632, 652, 672, 693, 713, 733, 754, 774, 795, 815, 835, 856, 876, 897, 917, 937, 958, 978, 978]
    return array[t-1]