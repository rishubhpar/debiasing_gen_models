...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, sample = True, male = 1, eyeglasses = 1, scale = [1500], timestep_list = [1,50], y=None, t_edit=400, hs_coeff=(1.0, 1.0), delta_h=None, ignore_timestep=False , use_mask=False, bt=1, attribute_list=[0,1,0,0], vanilla_generation=False):
        ...
        if not vanilla_generation:
            ...
            if conversion_timestep(timestep_list[0]) <= timesteps.item() <= conversion_timestep(timestep_list[1]):
                ...
                if sample:
                    ...
                    # print("going in")
                    # gradients = clip_classifier.gradients_mean(h,int(timesteps.item()), male=male, eyeglasses=eyeglasses, attribute_list=attribute_list,scale=scale, clip_latent=clip_latent,model_clip=model_clip, bt=bt)

                    # gradients = h_classification.gradients(h,int(timesteps.item()), scale)
        ...
        return h, None, delta_h, middle_h
